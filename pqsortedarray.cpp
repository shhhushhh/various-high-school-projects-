/**
  ...
  CS106B
  Assignment 4
  Creates a PriorityQueue that is sorted from highest to lowest.
  */
...
/**
 * @brief PQSortedArray::enqueue
 *      adds elem to elements, sorts elements, and resizes elements if it hits allocated capacity
 * @param elem
 *      the DataPoint to be added
 */
void PQSortedArray::enqueue(DataPoint elem) {
    ...
    sort();
    numItems++;
    elements[numItems - 1].name = elem.name;
    elements[numItems - 1].priority = elem.priority;
    sort();
    validateInternalState();
}

/**
 * @brief PQSortedArray::sort
 *      sorts the array, elements
 */
void PQSortedArray::sort() {
    if (numItems > 1) {
        for (int idx = numItems - 1; idx > 0; idx--) {
            if (elements[idx].priority > elements[idx - 1].priority) {
...
STUDENT_TEST("fillQueue and emptyQueue data tests"){
    PQSortedArray pq;
    TIME_OPERATION(15000, fillQueue(pq, 15000));
    TIME_OPERATION(15000, emptyQueue(pq, 15000));
